    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                queries = torch.tensor([x['prompt_input_ids'].tolist() for x in data], dtype=torch.int32).to(device)
                solutions = [x['solution'] for x in data]
                solutions = [x for i in range(args.rloo_k) for x in solutions]

                if self.args.max_prompt_length is not None:
                    queries = queries[:, -self.args.max_prompt_length :]
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    if isinstance(reward_model, nn.Module):
                        ...
                        output_reward_func = [reward if reward is not None else 0.0 for reward in output_reward_func]
