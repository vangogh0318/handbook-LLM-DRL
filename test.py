...
def load_custom_data(script_args, training_args, model_args, tokenizer) :
    ...
    def prepare_dataset(dataset, tokenizer):
        ...
        def tokenize(element):
            ...
            #return {"prompt_input_ids": prompt_ids, "lengths": prompt_len, "labels": solution_ids}
            #return {"input_ids": prompt_ids, "lengths": prompt_len}
...
def main(script_args, training_args, model_args):
    ...
    print("statrtttttttttttttttttttttt")
    pdb.set_trace()
    for data in dataloader:
        #queries = torch.Tensor([x['prompt_input_ids'].tolist() for x in data])
        queries = torch.tensor([x['prompt_input_ids'].tolist() for x in data], dtype=torch.int32)
